  ...
  return (
    <div className="overflow-x-auto">
      ...
      {relevantShows.map(show => {
        ...
        const isShipped = !!show.trackingNumber;
        const isShipByPast = !isShipped && daysUntilShipBy !== null && daysUntilShipBy < 0;
        const isShipByUrgent = !isShipped && daysUntilShipBy !== null && daysUntilShipBy >= 0 && daysUntilShipBy <= 3;
        const isShipByWarning = !isShipped && daysUntilShipBy !== null && daysUntilShipBy > 3 && daysUntilShipBy <= 7;
            ...
            <div className="flex-1 flex relative border-b border-border">
              ...
              {daysUntilShipBy !== null && shipBy && daysUntilShipBy >= 0 && daysUntilShipBy <= actualDaysToShow && (
                <div
                  className={cn(
                    ...
                    isShipped ? 'bg-success/40' : isShipByUrgent ? 'bg-error' : isShipByWarning ? 'bg-warning' : 'bg-success',
                  )}
                  style={{ left: daysUntilShipBy * 24 + 2 }}
                  title={isShipped ? `✓ Shipped · ${format(shipBy, 'MMM d')}` : `📦 Ship by: ${format(shipBy, 'MMM d')} (${daysUntilShipBy === 0 ? 'TODAY' : `${daysUntilShipBy}d`})`}
                >
              ...
              {daysUntilCutoff !== null && cutoff && daysUntilCutoff >= 0 && daysUntilCutoff <= actualDaysToShow && (
                <div
                  className={cn(
                    'absolute top-1/2 -translate-y-1/2 w-5 h-5 rounded flex items-center justify-center z-10',
                    isShipped ? 'bg-brand-purple/40' : 'bg-brand-purple',
                  )}
                  style={{ left: daysUntilCutoff * 24 + 2 }}
                  title={isShipped ? `✓ Delivered · ${format(cutoff, 'MMM d')}` : `🏭 Arrive by: ${format(cutoff, 'MMM d')} (${daysUntilCutoff === 0 ? 'TODAY' : `${daysUntilCutoff}d`})`}
                >
                  <Warehouse size={12} className="text-white" />
                </div>
              )}

              {/* Shipped indicator — replaces all warnings when tracking number exists */}
              {isShipped && (
                <div
                  className="absolute top-1/2 -translate-y-1/2 left-1 flex items-center gap-1 text-success"
                  title={`Shipped · Tracking: ${show.trackingNumber}`}
                >
                  <div className="w-4 h-4 rounded-full bg-success flex items-center justify-center">
                    <Check size={10} className="text-white" />
                  </div>
                  <span className="text-[10px] font-medium">Shipped</span>
                </div>
              )}
              ...
              {!isShipped && daysUntilCutoff !== null && cutoff && daysUntilCutoff < 0 && (
                <div
