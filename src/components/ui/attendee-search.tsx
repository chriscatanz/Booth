...
export function AttendeeSearch({ 
  ...
  const uniqueAttendees = useMemo(() => {
    ...
    for (const att of sorted) {
      ...
      if (key && !byEmail.has(key)) byEmail.set(key, att);
    }
    return Array.from(byEmail.values());
  }, [allAttendees]);

  const filteredAttendees = useMemo(() => {
    if (!search.trim()) return uniqueAttendees.slice(0, 10);
  ...
  const handleSelect = (att: Attendee) => {
    onSelect({ name: att.name, email: att.email });
    setSearch('');
    setIsOpen(false);
  };
  ...
  return (
    <div className="relative">
      ...
      <DropdownPortal
        ...
        className="bg-surface border border-border rounded-lg shadow-lg overflow-hidden max-h-64 overflow-y-auto"
      >
        {/* Add New */}
        <button
        ...
        {filteredAttendees.length > 0 && (
          <div className="py-1">
            <p className="px-4 py-1.5 text-xs font-medium text-text-tertiary uppercase">Previous Attendees</p>
              ...
              return (
                <button
                  ...
                  className={cn(
                    ...
                    isExcluded ? 'opacity-50 cursor-not-allowed' : 'hover:bg-bg-tertiary'
                  )}
                >
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="text-sm font-medium text-text-primary truncate">{att.name || 'Unnamed'}</p>
                    {att.email && <p className="text-xs text-text-tertiary truncate">{att.email}</p>}
                  </div>
                  {isExcluded && (
